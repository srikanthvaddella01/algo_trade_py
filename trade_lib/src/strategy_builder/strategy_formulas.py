...
def first_candle_type(df_bank_nifty_intraday_data_current, df_bank_nifty_intraday_data_previous, current_day_dict):
    if ((df_bank_nifty_intraday_data_current['cur_open'].values[0] <
         df_bank_nifty_intraday_data_previous['pre_low'].values[0]) & (
            df_bank_nifty_intraday_data_current['cur_close'].values[0] <
            df_bank_nifty_intraday_data_previous['pre_low'].values[0])):
        current_day_dict['candle_position'] = 'DOWN_OPEN_OUTSIDE_DOWN_CLOSE_OUT_SIDE'

    elif ((df_bank_nifty_intraday_data_current['cur_open'].values[0] <
           df_bank_nifty_intraday_data_previous['pre_low'].values[0]) & (
                  df_bank_nifty_intraday_data_current['cur_close'].values[0] >
                  df_bank_nifty_intraday_data_previous['pre_low'].values[0])):
        ...
        current_day_dict['candle_position'] = 'OPEN_IN_SIDE_DOWN_CLOSE_OUT_SIDE'
           ...
           df_bank_nifty_intraday_data_previous['pre_low'].values[0]) & (
                  df_bank_nifty_intraday_data_current['cur_close'].values[0] <
                  df_bank_nifty_intraday_data_previous['pre_high'].values[0])):
...
def current_first_candle_info(df_bank_nifty_intraday_data_current, current_day_dict):
    if df_bank_nifty_intraday_data_current['cur_open'].values[0] > \
        ...
        current_day_dict['candle_upper_wick'] = abs(round(
            (((df_bank_nifty_intraday_data_current['cur_open'].values[0] -
               df_bank_nifty_intraday_data_current['cur_high'].values[0])
              / (df_bank_nifty_intraday_data_current['cur_open'].values[0])) * 100), 2))

        current_day_dict['candle_lower_wick'] = abs(round(
            (((df_bank_nifty_intraday_data_current['cur_close'].values[0] -
               df_bank_nifty_intraday_data_current['cur_low'].values[0])
    ...
    current_day_dict['candle_width'] = abs(
        round((((df_bank_nifty_intraday_data_current['cur_high'].values[0] -
                 df_bank_nifty_intraday_data_current['cur_low'].values[0])
                / (df_bank_nifty_intraday_data_current['cur_high'].values[0])) * 100), 2))

    current_day_dict['candle_body'] = abs(
        round((((df_bank_nifty_intraday_data_current['cur_open'].values[0] -
                 df_bank_nifty_intraday_data_current['cur_close'].values[0])
                / (df_bank_nifty_intraday_data_current['cur_open'].values[0])) * 100), 2))
